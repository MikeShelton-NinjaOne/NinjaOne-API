...
function Test-TicketRow {
    ...
    # If a TicketID is provided, it must be digits only.
    # Letters, dashes, prefixes (e.g., 'LEG-1001', 'INC0042') are not accepted.
    $ticketId = [string]$Row.TicketID
    if (-not [string]::IsNullOrWhiteSpace($ticketId)) {
        if ($ticketId.Trim() -notmatch '^\d+$') {
            return "TicketID must contain only digits (got '$($ticketId.Trim())'). Strip any letters, dashes, or prefixes."
        }
    }
